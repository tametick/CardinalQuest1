...
class HxlQuadTree extends HxlRect {
	...
	public function new(X:Float, Y:Float, Width:Float, Height:Float, ?Parent:HxlQuadTree=null) {
		...
		var brush:HxlSprite = new HxlSprite().createGraphic(Width,Height,0xffffffff*HxlUtil.random());
		HxlState.screen.draw(brush,X+HxlG.scroll.x,Y+HxlG.scroll.y);//*/
		...
		if (Parent != null) {
			...
			_min = Math.floor((width + height)/(2*HxlUtil.quadTreeDivisions));
		}
	...
	public function add(Object:HxlObject, List:Int):Void {
		...
		if (Object._group) {
			var m:HxlObject;
			var members:Array<HxlObject> = (cast Object).members;
			var l:Int = members.length;
			for (i in 0...l) {
				m = cast( members[i], HxlObject);
				if ((m != null) && m.exists) {
					if (m._group) {
						add(m,List);
					} else if (m.solid) {
	...
	function addObject():Void {
		...
		if ((_ol > _l) && (_or < _mx)) {
			if ((_ot > _t) && (_ob < _my)) {
				if (_nw == null) {
					_nw = new HxlQuadTree(_l,_t,_hw,_hh,this);
				}
				_nw.addObject();
				return;
			}
			if ((_ot > _my) && (_ob < _b)) {
				if (_sw == null) {
					_sw = new HxlQuadTree(_l,_my,_hw,_hh,this);
				}
				_sw.addObject();
				return;
			}
		...
		if ((_ol > _mx) && (_or < _r)) {
			if ((_ot > _t) && (_ob < _my)) {
				if (_ne == null) {
					_ne = new HxlQuadTree(_mx,_t,_hw,_hh,this);
				}
				_ne.addObject();
				return;
			}
			if ((_ot > _my) && (_ob < _b)) {
				if (_se == null) {
					_se = new HxlQuadTree(_mx,_my,_hw,_hh,this);
				}
				_se.addObject();
				return;
			}
	 ...
	 * @return	Whether or not any overlaps were found.
	 */
	function overlapNode(?Iterator:HxlList=null):Bool {
		//member list setup
		var c:Bool = false;
		var co:HxlObject;
		var itr:HxlList = Iterator;
		if (itr == null) {
			if (_oa == A_LIST) {
				itr = _headA;
			} else {
		...
		if (itr.object != null) {
			...
			while (itr != null) {
				...
				if ( (_o == co) || !co.exists || !_o.exists || !co.solid || !_o.solid ||
					(_o.x + _o.width  < co.x + HxlUtil.roundingError) ||
					(_o.x + HxlUtil.roundingError > co.x + co.width) ||
					(_o.y + _o.height < co.y + HxlUtil.roundingError) ||
					(_o.y + HxlUtil.roundingError > co.y + co.height) ) {
					itr = itr.next;
					continue;
				}
				if (_oc == null) {
