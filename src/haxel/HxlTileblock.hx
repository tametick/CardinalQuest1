 ...
 */
class HxlTileblock extends HxlObject {
	/**
	...
	public function loadGraphic(TileGraphic:Class<Bitmap>,?Empties:Int=0) {
		...
		for (i in 0...numTiles) 	{
			if (HxlUtil.random()*(numGraphics+Empties) > Empties) {
				_rects.push(new Rectangle(_tileSize*Math.floor(HxlUtil.random()*numGraphics),0,_tileSize,_tileSize));
			} else {
				_rects.push(null);
			}
		}
	...
	function renderBlock() {
		...
		for (i in 0...rl) {
			...
			if (_flashRect != null) HxlGraphics.buffer.copyPixels(_pixels,_flashRect,_flashPoint,null,null,true);
		...
		if (HxlGraphics.showBounds) {
			...
			_bbRect.width = width;
			_bbRect.height = 1;
			HxlGraphics.buffer.fillRect(_bbRect,bbc);
